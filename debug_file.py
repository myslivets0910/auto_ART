import base64
import re

data11 = ['classified']
data22 = ['Classified']

# Преобразуем элементы из списков в строки и применяем необходимые методы для теста test_check_box
print(str(data11).replace(' ', '').replace('doc', '').replace('.', '').lower())
print(str(data22).replace(' ','').lower())

data1 = str(data11).replace(' ', '').replace('doc', '').replace('.', '').lower()
data2 = str(data22).replace(' ','').lower()

assert data11 == data22




# приводим к одному виду файл при загрузке и сравнении для теста test_upload_file
data111 = 'C:\Users\kabac\PycharmProjects\auto_ART\filetest616.txt'
data222 = 'C:\fakepath\filetest616.txt'

print(data111.split('\\'))
print(data222.split('\\'))

assert (data111.split('\\'))[-1] == (data222.split('\\'))[-1]


# приводим к одному виду файл при загрузке и сравнении для теста test_download_file

data1111 = 'data:image/jpeg;base64,'
data_decod = base64.b64decode(data1111)
offset = data_decod.find(b'\xff\xd8')
data_andecod = base64.b64decode(data_decod)

print(data_andecod)




subject = {"Hindi", "English", "Maths", "Physics", "Chemistry", "Biology", "Computer Science", "Commerce", "Accounting", "Economics", "Arts", "Social Studies", "History", "Civics" }

state = {"NCR", "Uttar Pradesh", "Haryana", "Rajasthan"}

PASSED [100%]Person(full_name='Савелий Крылова Власович', firstname='Климент', lastname='Некрасов', age=15, salary=9201, department='Хлебороб', email='ivanna_13@example.org', current_address='к. Шахты, бул. Советов, д. 2 стр. 2, 005118', permanent_address='к. Комсомольск-на-Амуре, ул. Колхозная, д. 799, 979833', mobile='0343041566074')
['Климент Некрасов', 'ivanna_13@example.org', 'Other', '0343041566', '17 June,2025', 'Physics', 'Reading', 'filetest457.txt', 'к. Шахты, бул. Советов, д. 2 стр. 2, 005118', 'Uttar Pradesh Agra']


data = 'position: relative; left: 0px; top: 20px;', 'position: relative; left: 0px; top: 61px;'
re_data1 = data
top = re.findall(r'\d[0-9]|\d', data.split(';')[2])
print(top)

